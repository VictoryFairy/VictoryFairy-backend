...
export class AuthController {
  ...
  async localLogin(@Body() body: LoginLocalUserDto, @Res() res: Response) {
    const { email, id, support_team } =
      await this.accountApplicationCommandService.loginLocalUser(body);
  ...
  async handleSocialLogin(
    ...
    const { user, isNewUser } =
      await this.accountApplicationCommandService.loginSocialUser(
        sub,
        providerEmail,
        provider,
      );
  ...
  async reissueAcToken(
    ...
    const { email, id, support_team } =
      await this.accountApplicationQueryService.getUserWithSupportTeam(userId);
